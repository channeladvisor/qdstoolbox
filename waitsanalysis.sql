...
SELECT  
	 [ObjectName] = ISNULL([s].[name]+''.''+[o].[name],'''')
	,[PlanID] = 
	...
	{@IncludeQueryText},COMPRESS([qsqt].[query_sql_text]) AS [QueryText]
	{@NotIncludeQueryText},NULL AS [QueryText]
...
{@IncludeQueryText}INNER JOIN [{@DatabaseName}].[sys].[query_store_query_text] [qsqt]
{@IncludeQueryText}ON [qsq].[query_text_id] = [qsqt].[query_text_id]
WHERE 
	(
...
BEGIN
	SET @SqlCmd = REPLACE(@SqlCmd,	'{@IncludeQueryText}',		'--')
	SET @SqlCmd = REPLACE(@SqlCmd,	'{@NotIncludeQueryText}',	'')
END
...
BEGIN
	...
	SELECT
		...
		FOR XML PATH(''QueryWaitParameters''), ROOT(''Root'')
		)	AS [Parameters]'
	...
	SET @SqlCmdIndex = REPLACE(@SqlCmdIndex, '{@ObjectNameName}',	ISNULL(@ObjectName, ''))
